...
class UFOTradingEngine:
    ...
    def __init__(self, config):
        ...
        # UFO Session Timing Configuration (from config.ini)
        # Parse config values properly (strip comments and whitespace)
        def parse_config_time_int(key, default):
            value = config['trading'].get(key, str(default))
            # Remove inline comments and whitespace
            clean_value = value.split('#')[0].strip()
            return int(clean_value)
        
        self.session_start_hour = parse_config_time_int('session_start_hour', 8)
        self.session_start_minute = parse_config_time_int('session_start_minute', 0)
        self.session_end_hour = parse_config_time_int('session_end_hour', 20)
        self.session_end_minute = parse_config_time_int('session_end_minute', 0)
        self.weekend_close_hour = parse_config_time_int('weekend_close_hour', 21)
        self.weekend_close_minute = parse_config_time_int('weekend_close_minute', 0)
    ...
    def should_trade_now(self):
        ...
        UFO methodology: Respect the configurable session end time (8 PM GMT default)
        """
        ...
        # Use GMT for UFO session end rule
        current_time_gmt = now_utc.time()
        
        # UFO RULE: Check if we're within the configured daily trading window
        session_start_time = time(self.session_start_hour, self.session_start_minute)
        session_end_time = time(self.session_end_hour, self.session_end_minute)
        
        print(f"🕐 DEBUG: Current GMT time: {current_time_gmt}")
        print(f"📅 DEBUG: Session window: {self.session_start_hour:02d}:{self.session_start_minute:02d} - {self.session_end_hour:02d}:{self.session_end_minute:02d} GMT")
        
        # Check if we're within the daily trading window
        if session_start_time <= session_end_time:
            # Normal case: start time is before end time (e.g., 8 AM - 8 PM)
            within_session = session_start_time <= current_time_gmt < session_end_time
        else:
            # Edge case: session crosses midnight (e.g., 10 PM - 6 AM next day)
            within_session = current_time_gmt >= session_start_time or current_time_gmt < session_end_time
        
        if not within_session:
            print(f"⛔ UFO SESSION CLOSED: Outside trading window")
            return False, f"Outside UFO trading window ({self.session_start_hour:02d}:{self.session_start_minute:02d}-{self.session_end_hour:02d}:{self.session_end_minute:02d} GMT)"
        
        # Close before weekend (configurable)
        if current_weekday == 4 and current_time_london >= time(self.weekend_close_hour, self.weekend_close_minute):
            return False, f"Weekend closure - Friday {self.weekend_close_hour:02d}:{self.weekend_close_minute:02d} London"
        
        # Define trading sessions (only if within UFO session window)
        asian_session = time(23, 0) <= current_time_london or current_time_london <= time(8, 0)
        london_session = time(8, 0) <= current_time_london <= time(16, 0)
        ny_session = time(13, 0) <= current_time_london <= time(22, 0)
    ...
    def should_close_for_session_end(self, economic_events=None):
        ...
        # Close before weekend (configurable)
        if current_weekday == 4 and current_time_london >= time(self.weekend_close_hour, self.weekend_close_minute):
            return True, f"Weekend closure - Friday {self.weekend_close_hour:02d}:{self.weekend_close_minute:02d} London"
            
        # Close at configured session end time - UFO methodology end of analysis period
        if current_time_gmt >= time(self.session_end_hour, self.session_end_minute):
            return True, f"End of UFO analysis period ({self.session_end_hour:02d}:{self.session_end_minute:02d} GMT)"
