...
class MT5DataCollector:
    ...
    def connect(self):
        ...
        if not mt5.initialize(path=self.path, login=self.login, password=self.password, server=self.server):
            ...
            print(f"Failed to initialize MT5: {error_code} - {error_desc}")
            self.connected = False
            return False
    ...
    def get_historical_data(self, symbol, timeframe, num_bars=1000):
        ...
        # Ensure symbol has the correct suffix for MT5
        if not symbol.endswith('-ECN'):
            symbol = symbol + '-ECN'
        ...
        if not validated_symbol:
            print(f"❌ Invalid currency pair for historical data: {symbol}")
            return None
        symbol = validated_symbol  # Use validated symbol
        
        # Ensure connection before attempting data retrieval
        max_retries = 3
        for retry in range(max_retries):
            # Check and restore connection if needed
            if not self.connected or mt5.account_info() is None:
                print(f"Connection lost, reconnecting... (attempt {retry + 1}/{max_retries})")
                if not self.connect():
                    continue
            ...
            if rates is not None:
                ...
                error_code, error_desc = mt5.last_error()
                print(f"Failed to get rates for {symbol} (attempt {retry + 1}): ({error_code}, '{error_desc}')")
                
                # If it's a connection error, force reconnection
                if error_code == -10004:  # No IPC connection
                    self.connected = False
                    mt5.shutdown()
    ...
    def get_live_data(self, symbol, timeframe, prev_time=None):
        ...
        validated_symbol = self.pair_validator.validate_and_normalize_pair(symbol)
        if not validated_symbol:
            print(f"❌ Invalid currency pair for live data: {symbol}")
            return None
        symbol = validated_symbol  # Use validated symbol
    ...
    def get_daily_ufo_data(self, symbol, timeframe=None):
        ...
        validated_symbol = self.pair_validator.validate_and_normalize_pair(symbol)
        if not validated_symbol:
            print(f"❌ Invalid currency pair for UFO data: {symbol}")
            return None
        symbol = validated_symbol  # Use validated symbol
