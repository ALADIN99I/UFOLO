...
class TradeExecutor:
    ...
    def execute_trade(self, symbol, trade_type, volume, price, sl=0, tp=0, comment=""):
        ...
        if not validated_symbol:
            print(f"❌ Invalid currency pair: {symbol}")
            return None
    ...
    def execute_portfolio(self, trades):
        ...
        results = []
        for trade in trades:
            # Validate symbol in trade dict
            validated_symbol = self.pair_validator.validate_and_normalize_pair(trade.get('symbol', ''))
            if not validated_symbol:
                print(f"❌ Skipping trade with invalid symbol: {trade.get('symbol', 'None')}")
                results.append(None)
                continue
    ...
    def close_all_positions(self):
        """
        Alias for close_all_trades() for compatibility.
        """
